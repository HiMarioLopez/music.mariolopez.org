...
interface UseRecommendationSearchProps {
    onRecommend: (songTitle: string) => void;
}

export const useRecommendationSearch = ({ onRecommend }: UseRecommendationSearchProps) => {
    ...
    const handleSongSelect = useCallback((result: SearchResult) => {
        // Track focus before selection
        trackFocus();
    ...
    const hasResults = useMemo(() => {
        return hintResults.length > 0 || songResults.length > 0;
    }, [hintResults.length, songResults.length]);
