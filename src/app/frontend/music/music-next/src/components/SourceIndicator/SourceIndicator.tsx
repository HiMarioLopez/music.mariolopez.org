 ...
 */
export const SourceIndicator = memo(
  ({ source, size = "small", url, onClick }: SourceIndicatorProps) => {
    // Default to Apple Music if no source is provided (original data source)
    // Only hide indicator if explicitly set to 'unknown'
    const actualSource: MusicSource =
      source && source !== "unknown" ? source : "apple";
