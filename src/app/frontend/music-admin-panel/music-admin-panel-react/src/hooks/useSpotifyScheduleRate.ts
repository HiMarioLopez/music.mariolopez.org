...
import { updateSpotifyScheduleRate, getSpotifyScheduleRate } from '../utils/api';

export type RateUnit = 'minute' | 'minutes' | 'hour' | 'hours' | 'day' | 'days';
type RateType = 'simple' | 'cron';

interface SpotifyScheduleRateState {
    rateType: RateType;
    value: string;
    unit: RateUnit;
...
interface SpotifyScheduleRateActions {
...
export function useSpotifyScheduleRate(): [SpotifyScheduleRateState, SpotifyScheduleRateActions] {
    ...
    const fetchCurrentRate = async () => {
        try {
            setIsLoading(true);
            const response = await getSpotifyScheduleRate();
            ...
            console.error('Error fetching Spotify schedule rate:', error);
            setStatus('Error: Failed to fetch current Spotify schedule rate');
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleUpdate = async () => {
        try {
            if (rateType === 'simple') {
                ...
                await updateSpotifyScheduleRate(`rate(${value} ${unit})`);
            } else {
                ...
                await updateSpotifyScheduleRate(`cron(${cronExpression})`);
            }
            setStatus('Spotify schedule rate updated successfully');
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : 'Failed to update Spotify schedule rate';
