...
/**
 * Lambda handler for retrieving the Spotify history schedule rate from Parameter Store
 */
export const handler = wrapHandler<APIGatewayProxyEvent, APIGatewayProxyResult>(
  { serviceName: 'get-spotify-schedule-rate' },
  async (event, context, utils) => {
    ...
    if (!rate) {
      utils.logger.warn('Spotify schedule rate parameter not found', { parameterName });
      ...
      return utils.createErrorResponse(
        ...
        'Spotify schedule rate parameter not found'
      );
    }

    utils.logger.info('Successfully retrieved Spotify schedule rate', { rate });
