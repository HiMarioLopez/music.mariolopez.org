...
const MIN_SONG_LIMIT = 1;
const MAX_SONG_LIMIT = 50;

/**
 * Lambda handler for updating Spotify history song limit parameter
 */
export const handler = wrapHandler<APIGatewayProxyEvent, APIGatewayProxyResult>(
  { serviceName: 'set-spotify-song-limit' },
  async (event, context, utils) => {
    ...
    if (
      ...
      utils.logger.warn('Invalid Spotify song limit provided', { songLimit });
      utils.metrics.addMetric('ValidationError', MetricUnit.Count, 1);
      return utils.createErrorResponse(
        event,
    ...
    utils.logger.info('Spotify song limit updated successfully', { songLimit });
    utils.metrics.addMetric('UpdateSuccess', MetricUnit.Count, 1);

    return utils.createSuccessResponse(event, {
      message: 'Spotify song limit updated successfully',
      songLimit,
    });
  }
);
